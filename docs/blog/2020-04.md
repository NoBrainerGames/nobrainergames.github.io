...
A language like D -- which interfaces with C++ through the ABI -- requires the programmer to define all of the fields of the value:
...
It may not be obvious in this simple example, but this detail makes a world of difference when working with large codebases. Most non-trivial C++ data structures would contain fields that are themselves C++ data structures, typically requiring massive swaths of boilerplate code. With Nim, simply declaring a type's name allows that type to be used immediately, its innards needing to be defined only when explicitly accessed.
